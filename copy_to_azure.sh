...
Copy an Amazon AMI to an Azure storage account and create a shared image
 ...
 Usage: $(basename $0) -a AMI_ID -b BUCKET_NAME -s STORAGE_ACCOUNT -c STORAGE_ACCOUNT_CONTAINER -r RESOURCE_GROUP -i IMAGE_GALLERY_NAME -n IMAGE_NAME -v IMAGE_VERSION
 ...
 Options:
   ...
   -r    Resource group of the shared image gallery
...
while getopts ":a:b:s:c:r:i:n:v" opt; do
    case ${opt} in
        a)
            AMI_ID="${OPTARG}"
            ;;
...
export_task_id=$(aws ec2 export-image \
              ...
              --s3-export-location "S3Bucket=${BUCKET_NAME},S3Prefix=exports/" | jq ".ExportImageTaskId")

# wait for export image task to finish
while ! aws ec2 describe-export-image-tasks --export-image-task-ids "${export_task_id}" --query "ExportImageTasks[0].Status" | grep -q "completed" ; do
